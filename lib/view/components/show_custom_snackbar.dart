...
class CustomSnackbar{
  static  showCustomSnackbar({required List<String>errorList,required List<String> msg,required bool isError,int duration=5}){
    ...
    if(isError){
      if(errorList.isEmpty ?? true){
        ...
        for (var element in errorList) {
          message=message.isEmpty ?? true?'${message.tr}${element.tr}':"${message.tr}\n${element.tr}";
        }
      }
    ...
    message = message.isEmpty ?? true? isError?MyStrings.failedToUpdateProfile:MyStrings.profileUpdatedSuccessfully : message;
    Get.rawSnackbar(
